 ...
 * 
 * Solution: I use 2 heaps(O(n log n)) and the median can always be accessed in
 * O(1) time. At any time I keep the two heaps balanced in size (they differ at
 * most by 1). Adding a new element is O(log n).
 * 
 */
public class MedianHeap {

	// stores all the numbers less than the current median in a maxheap, i.e
	// median is the maximum, at the root
	private PriorityQueue<Integer> maxheap;
	// stores all the numbers greater than the current median in a minheap, i.e
	// median is the minimum, at the root
	...
	/**
	 * Comparator for the minHeap, smallest number has the highest priority,
	 * natural ordering
	 */
	 ...
	 */
	public MedianHeap() {
		// Initialise maxheap and minheap with appropriate comparators
	...
	public void insert(int n) {
		...
		if (isEmpty()) {
			...
			// if n is greater than current median, add to minheap
			else {
				minheap.add(n);
			}
		}
	...
	private void fixChaos() {
		// if sizes of heaps differ by 2, then it's a chaos, since median must
		// be the middle element
		if (Math.abs(maxheap.size() - minheap.size()) > 1) {
			// check which one is the culprit and take action by kicking out the
			// root from culprit into victim
	...
	public double median() {
		// if total size(no. of elements entered) is even, then median is the
		// average of the 2 middle elements
		// i.e, average of the root's of the heaps.
		if (maxheap.size() == minheap.size()) {
			return ((double) maxheap.peek() + (double) minheap.peek()) / 2;
		}
		// else median is middle element, i.e, root of the heap with one element
		// more
	...
	public void test(int N) {
		...
		if (N % 2 == 0) {
			System.out.println("Calculated Median is :" + (array[N / 2] + array[(N / 2) - 1]) / 2.0);
		} else {
			System.out.println("Calculated Median is :" + array[N / 2] + "\n");
		}
	}
